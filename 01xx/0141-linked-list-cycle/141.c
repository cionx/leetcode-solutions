 ...
 *   NULL, in which case we return `true`.
 * - If the list has a cycle, then the slow pointer enters this cycle at some
 *   point in time. The fast pointer is then also trapped in the cycle. The fast
 *   pointer will then overtake the slow pointer at some future point in time.
 *   As the speed difference between the two pointers is exactly one node, this
 *   will entail that the two pointers meet at some node. We can then return
 *   `true`.
